...
class Audio(arcade.gui.UIManager):
    ...
    def setup(self, on_close, on_change) -> None:
        ...
        slider_master = make_slider(value=self._state.audio_volumes.volume_master, min_value=0, max_value=100)
        slider_master.on_change = self.on_change_volume_master

        label_sound = make_label(text=_('Sound volume'))
        slider_sound = make_slider(value=self._state.audio_volumes.volume_sound, min_value=0, max_value=100)
        slider_sound.on_change = self.on_change_volume_sound

        label_speech = make_label(text=_('Speech volume'))
        slider_speech = make_slider(value=self._state.audio_volumes.volume_speech, min_value=0, max_value=100)
        slider_speech.on_change = self.on_change_volume_speech

        label_music = make_label(text=_('Music volume'))
        slider_music = make_slider(value=self._state.audio_volumes.volume_music, min_value=0, max_value=100)
        slider_music.on_change = self.on_change_volume_music

        label_subtitle_size = make_label(text=_('Size of subtitles'))
        slider_subtitle_size = make_slider(value=self._state.subtitle_size, min_value=14, max_value=20)
